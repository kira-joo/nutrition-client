 ...
 * `src/common/books/book-physical-order.ts` — the RTL physical-book-order
 * invariant. Every renderer that needs left/right (the print template
 * there, the Flipbook here) imports this instead of hard-coding a side,
 * so the binding direction can never drift between the two apps.
 *
 ...
 * of the gutter — so page 1 is a LEFT-hand page, odd pages sit on the
 * left, even pages sit on the right. Matches InDesign's own
 * right-to-left binding convention.
 */
