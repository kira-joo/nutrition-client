 ...
 * `src/server/books/render/dr-omnia-book-v1/template.css.ts` — same
 * brand colors/classes, so a Flipbook page is visually identical to the
 * PDF/print output. Keep both in sync by hand on any visual change.
 */
export const BRAND_COLORS = {
  ...
  /** The ordinary-page inner frame's accent — a warm gold-bronze, distinct from the cool green everything else uses. */
  frameGold: "rgba(176, 141, 79, 0.45)",
};

/**
 * The template's botanical artwork, served from `public/` like any other
 * static asset — one source of truth for the path so a caller never
 * hand-types it. nutrition-staff embeds the same bytes as a base64 data
 * URI instead (`art/chapter-background.ts`), since Puppeteer's
 * `setContent` has no base URL to resolve a relative path against.
 */
...
export interface BuildTemplateCssOptions {
   ...
   * to resolve a relative path against). Reused across covers, the back
   * cover, and every chapter opener — one asset, one visual language.
   * Optional so an unmigrated caller still produces valid (plain-color)
   * CSS rather than a broken url().
   */
...
.book-page-scope, .book-page-scope * { margin: 0; padding: 0; box-sizing: border-box; }
...
/* PAGE_FOOTER_NOTE — a generic, reusable "pin to the bottom of whatever
   page this lands on" block (see paginate-book.ts's "pageFooterNote"
   FragmentKind branch), reusing the title page's legal-footer visual
   language (divider above, muted smaller type). The paginator only ever
   emits the .book-page-content-body wrapper when a footer note actually
   landed on this page, so :has() keeps every other page a plain
   flow-root box, completely unaffected. .book-page-content-body's flex:1
   absorbs all the space ABOVE the footer note — the same technique
   .book-title-page-main uses — which is what reserves the footer's own
   height automatically instead of position:absolute. Hand-synced with
   nutrition-staff's identical rules. */
...
/* Chapter openers are real, numbered pages (they count toward pagination/
   ...
   running head — matching the approved reference, which shows neither.
   The :has() selector keeps this a pure CSS rule rather than a new
   "unnumbered but still counted" flag on the page model. Hand-synced with
   nutrition-staff's identical rule. */
.book-page:has(.book-chapter-opener) .book-folio,
.book-page:has(.book-chapter-opener) .book-running-head {
  display: none;
}

/* An ordinary printed page's inner frame — a thin decorative rule set IN
   the margin, between the trim edge and the text block, not around the
   text block itself. Only ordinary content pages get it; the cover,
   chapter openers, and the back cover carry their own full-bleed artwork
   and would look wrong with a frame drawn over it. Hand-synced with
   nutrition-staff's identical rule. */
...
.book-callout { border-radius: 2mm; padding: 3mm 4mm; margin: 3mm 0; }
...
figure.book-image { margin: 3mm 0; text-align: center; }
...
/* position:absolute + inset:0 fills the WHOLE .book-page box edge to
   ...
   side and outerMm on the outer side — two DIFFERENT values.
   Cancelling both with one -outerMm margin left a real, visible gap on
   whichever side carries gutterMm instead — the bug behind the cover
   rendering with a visible white margin instead of true full-bleed.
   Hand-synced with nutrition-staff's identical rule. */
...
/* "uploaded" mode (see the client Book domain type's coverMode/
   backCoverMode) — the doctor's own finished A5 cover/back-cover becomes
   the WHOLE page, full-bleed. Deliberately no flex centering, no color,
   no text-shadow rules: nothing is ever rendered inside this element at
   all (see renderCoverPage/renderBackCoverPage's "uploaded" branch) — the
   image itself already contains whatever design/text it needs, so this
   is pure background positioning and nothing else. Hand-synced with
   nutrition-staff's identical rule. */
...
/* A full-bleed page, not a content block sharing a page with what
   follows (see paginate-book.ts — chapter openers are now emitted as
   "singlePage" fragments). position:absolute + inset:0 fills the whole
   .book-page box, including its padding, because .book-page is already
   position:relative — simpler than .book-cover's negative-margin trick
   and doesn't depend on outerMm being symmetric on both sides. If a
   chapter has its own coverImage, that takes over the entire background
   instead of the template artwork (template-with-content-override,
   matching the plan's "template owns the visual language, content fills
   it in"); a scrim keeps the always-white text legible over an arbitrary
   uploaded photo the same way it already is over the template's own dark
   artwork. Hand-synced with nutrition-staff's identical rule. */
.book-chapter-opener {
  position: absolute;
  inset: 0;
  overflow: hidden;
...
.book-toc-entry { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 3mm; gap: 2mm; cursor: pointer; }
